...
function pull() {
  ...
  // 균균 연구소 도감에 추가
  const gyunLabCollection = JSON.parse(localStorage.getItem('gyun_lab_collection') || '{}');
  const gyunId = char.name.toLowerCase().replace(/\s+/g, '-');
  gyunLabCollection[gyunId] = (gyunLabCollection[gyunId] || 0) + 1;
  localStorage.setItem('gyun_lab_collection', JSON.stringify(gyunLabCollection));
...
function startAutoRoll() { if (autoRollInterval) return; autoRollInterval = setInterval(pull, 1); }
...
if (e.key.toLowerCase() === 'f' && curZone && !inGacha && !inWorship && !inBattle) {
  ...
  else if (curZone.id === 'gyun-research-lab') {
    changeMap('gyun-research-lab');
  }
